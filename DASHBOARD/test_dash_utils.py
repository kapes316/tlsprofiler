...
class Dashboard_util2:
    ...
    def onLoad_tls_stats_options(self):
        ...
        tls_stats = [
            ...
            {'label': "TLS 1.3 Support", 'value': "TLS1.3"},
            {'label': 'stat_table', 'value': 'stat_table'},
        ]
    ...
    def signature_from_certificate_table(self, cert_table=None, sha_hash=None):
        sql_conn = sqlite3.connect(DB_file)
        cur = sql_conn.cursor()
        query = "select sig_algo from {0} where sha_hash=\"{1}\"".format(cert_table, sha_hash)
        cur.execute(query)
        result = cur.fetchall()
    ...
    def convert_tuple_to_dict(self, row_info, main_table=main_table, cert_table=certificate_table):
        sha_hash = row_info[-1]
        sig_algo = self.signature_from_certificate_table(cert_table=certificate_table, sha_hash=sha_hash)
        pprint(sig_algo)
        row_dict = { "id" : row_info[0], "host" : row_info[2],
                     "date": row_info[1], "cipher": row_info[3],
                     "tls_version": row_info[4], "certificate" : sig_algo}
        pprint(row_dict)
        return row_dict
    ...
    def get_tls_main_table_entries(self, table=main_table):
        sql_conn = sqlite3.connect(DB_file)
        cur = sql_conn.cursor()
        row_count = self.get_max_rows_count_table(main_table)
        pprint(row_count)
        ROW = list()
        row_count = 5
        for i in range(1, row_count+1):
            row_info = self.get_a_row_info(table=main_table, index=i)
            pprint(row_info[0])
            pprint(type(row_info[0]))
            tup = row_info[0]
            pprint(tup[-1])
            pprint(type(tup[-1]))
            row_dict = self.convert_tuple_to_dict(row_info[0], main_table=main_table, cert_table=certificate_table)
            ROW.append(row_dict)

        pprint(ROW)

        #result is rows as tuples in a list for the entire DB
